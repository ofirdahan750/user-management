    ...
    catchError((error: HttpErrorResponse) => {
      ...
      if (!skipToast || error.error instanceof ErrorEvent) {
        ...
        if (error.error instanceof ErrorEvent) {
          ...
          const apiError: ApiError | null = apiErrorService.isApiError(error.error) ? error.error : null;
          
          switch (error.status) {
            case 400:
              errorMessage = apiErrorService.getErrorMessage(apiError, ERROR_MESSAGES.BAD_REQUEST);
              break;
            case 401:
              // Check if it's an account locked scenario
              const errorMsg = apiError ? (apiErrorService.getErrorMessage(apiError, '')).toLowerCase() : '';
              if (errorMsg.includes('locked') || errorMsg.includes('lockout')) {
                errorMessage = MESSAGES.ACCOUNT_LOCKED;
              } else {
                errorMessage = apiErrorService.getErrorMessage(apiError, MESSAGES.UNAUTHORIZED);
              }
              break;
            case 403:
              errorMessage = apiErrorService.getErrorMessage(apiError, MESSAGES.UNAUTHORIZED);
              break;
            case 404:
              errorMessage = apiErrorService.getErrorMessage(apiError, ERROR_MESSAGES.RESOURCE_NOT_FOUND);
              break;
            case 409:
              errorMessage = apiErrorService.getErrorMessage(apiError, MESSAGES.DUPLICATE_EMAIL);
              break;
            case 423: // Account Locked status code
              errorMessage = MESSAGES.ACCOUNT_LOCKED;
              break;
            case 429:
              errorMessage = apiErrorService.getErrorMessage(apiError, ERROR_MESSAGES.TOO_MANY_REQUESTS);
              break;
            case 500:
              errorMessage = apiErrorService.getErrorMessage(apiError, ERROR_MESSAGES.SERVER_ERROR);
              break;
            default:
              // Check for account locked in error message
              const defaultErrorMsg = apiError ? apiErrorService.getErrorMessage(apiError, '').toLowerCase() : '';
              if (defaultErrorMsg.includes('locked') || defaultErrorMsg.includes('lockout')) {
                errorMessage = MESSAGES.ACCOUNT_LOCKED;
              } else {
                errorMessage = apiErrorService.getErrorMessage(apiError, MESSAGES.ERROR);
              }
          }
        }
